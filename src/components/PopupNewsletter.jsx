...
export default function PopUpNewsletter() {
  const { open, setOpen, randomClick, setRandomClick, timestamp, setTimestamp, newsletter, setNewsletter, updateRandomClick } = useNewsletter();
        ...
        <div className="popup-newsletter-content bg-white rounded-3 p-3">
          ...
          <form className="d-flex flex-column gap-2" onSubmit={handleSubmit}>
            <div className="form-group d-flex flex-column gap-2 p-3">
              ...
              <input type="text" placeholder="Cognome" value={form.surname} onChange={(e) => setForm({ ...form, surname: e.target.value })} />
              <label htmlFor="email">Email</label>
              <input type="email" placeholder="Email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
              <label htmlFor="accept-terms">Accetto i termini e le condizioni</label>
              <div className="form-check">
                <input type="checkbox" className="form-check-input" id="accept-terms" checked={form.acceptTerms} onChange={(e) => setForm({ ...form, acceptTerms: e.target.checked })} />
