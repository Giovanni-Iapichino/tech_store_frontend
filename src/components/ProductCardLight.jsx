...
export default function ProductCardLigth({ product }) {
  if (product.promotions) {
    const promotion_in_corso = product.promotions.filter((promotion) => promotion.promo_state === "in corso");
    if (promotion_in_corso.length > 0) {
      product.promotions = promotion_in_corso[0];
    } else {
      ...
      <div className="position-absolute top-0 end-0 p-1">
        {product.promotions && (
          <div className="fs-6 bg-danger text-white rounded-3 p-1">
            <span>{product.promotions.discount}%</span>
          </div>
        )}
      </div>
      {product.promotions && (
        <div className="promo_state position-absolute top-0 start-0 p-1">
          {product.promotions.promo_state === "in corso" ? (
            <div className="promotion-item-content d-flex flex-column gap-2">
              <span>in corso</span>
            </div>
          ) : (
            <div className="promotion-item-content d-flex flex-column gap-2">
              <span>dal {new Date(product.promotions.start_date).toLocaleDateString()}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="promotion-item-content d-flex flex-column gap-2">
        ...
        {product.promotions ? (
          <span className="d-flex flex-row align-items-center justify-content-center gap-2 w-100">
            ...
            <span className="discounted-price text-danger fw-bold">{product.promotions.discount_price}€</span>
          </span>
        ) : (
          <></>
        )}
